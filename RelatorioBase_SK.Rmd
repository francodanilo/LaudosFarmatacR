...
for (c in names(subset(dados, select = -c(Parc, Bloco, Tratamento, Trat, Regr, Dose)))) {
  ...
  if(legendas[c,"Tukey"] == "Sim") {
    ...
    if(sum(dados[[c]]) == 0) {
      ...
      dados2 = dados[3]
      dados2$Bloco = dados$Bloco
      dados2$dadosT = dadosT
      sk = ea1(dados2, design = 2, alpha = 0.05, list = FALSE, p.adjust=5, plot=3)
      ...
      tab.Tukey = dados %>% 
        ...
        arrange(desc(mean)) %>% 
        mutate(ltrs = sk$`Adjusted means`$scott_knott) %>% 
        arrange(Tratamento) %>% 
        as.data.frame()
      ...
      cat(kable(tab.Tukey[,c(1,2,5,6)], col.names = c("Tratamentos",legendas[c,"ColunaEfic"],"",legendas[c,"AouR"]), caption = legendas[c,"legendaTukey"]) %>% 
            kable_styling(bootstrap_options = "striped", full_width = FALSE) %>%
            footnote(general_title = "Informações estatísticas", general = c(paste("Teste F = ", round(Fvalue,digits = 3)," Pr(>F) = ",round(pvalue,digits = 3), "; CV (%) =  ", round(cv,digits = 2), sep = " "),"Médias seguidas das mesmas letras, não diferem entre si pelo teste de Scott-Knott com 5% de probabilidade de erro.",legendas[c,"rodapeTukey"])))
      cat("\n","\n",'<div style="page-break-after: always"></div>')
    }
