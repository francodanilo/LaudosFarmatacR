...
estado = read_state(code_state = geral$info[10], year=2020)
...
# Contratada
Farm Assistência Técnica S/S Ltda

CNPJ 05.814.979/0001-55

Estm Bebedouro a Terra Roxa sn Km 6
...
for (c in names(select(dados, -Parc, -Bloco, -Tratamento, -Trat, -Regr, -Dose))) {
...
for (c in names(select(dados, -Parc, -Bloco, -Tratamento, -Trat, -Regr, -Dose))) {
  ...
  if(legendas[c,"Tukey"] == "Sim") {
    ...
    if(sum(dados[[c]]) == 0) {
      ...
      dadosT = sqrt(dados[[c]] + mean(dados[[c]]))
      anava = aov(dadosT ~ Tratamento + Bloco, data = dados)
      TK = TukeyHSD(anava, conf.level = 0.95, ordered = FALSE)
      ...
      if(legendas[c,"Boxp"] == "Sim") {
        ...
        gbp = ggplot(dados, aes(x=Tratamento, y=dados[[c]])) + 
        geom_boxplot() + 
        labs(x = legendas[c,"xBoxplot"], y=legendas[c,"yBoxplot"]) +
        geom_jitter(height = 0, width = 0.1, shape = 21, size = 2,color = "blue", fill = "blue", alpha = 0.5) +
        stat_summary(fun.y=mean, geom="point", shape=23, size=3, color = "red", fill = "red") +
        theme_bw() + 
        ...
        geom_label(data = tab.Tukey, aes(label = mean, x = Tratamento, y = mean), vjust = 1.3, hjust = 1) +
      ...
      cat(kable(tab.Tukey[,c(1,2,5,6)], col.names = c("Tratamentos",legendas[c,"ColunaEfic"],"",legendas[c,"AouR"]), caption = legendas[c,"legendaTukey"]) %>% 
            kable_styling(bootstrap_options = "striped", full_width = FALSE) %>%
            footnote(general_title = "Informações estatísticas", general = c(paste("Teste F = ", round(Fvalue,digits = 3)," Pr(>F) = ",round(pvalue,digits = 3), sep = " "), paste("CV (%) =  ", round(cv,digits = 2), sep = " "),"Médias seguidas das mesmas letras, não diferem entre si pelo teste de Tukey com 5% de probabilidade de erro",legendas[c,"rodapeTukey"])))
      cat("\n","\n",'<div style="page-break-after: always"></div>')
    }
...
for (j in names(select(dados, -Parc, -Bloco, -Tratamento, -Trat, -Regr, -Dose))) {
